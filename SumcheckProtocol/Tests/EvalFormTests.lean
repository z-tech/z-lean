...
`SumcheckProtocol/Src/EvalForm.lean`) and can be cross-checked against `effsc`'s
output for the same polynomial / domain / round / challenge prefix.

## Convention recap (matches `SumcheckProtocol/Src/EvalForm.lean`)
...
/-- SumcheckProtocol consistency at round 0: G_0(0) + G_0(1) = honest claim. -/
example :
...
* SumcheckProtocol consistency: G_0(0) + G_0(1) = 2 + 4 = 6 = claim. -/

...
* `SumcheckProtocolStatementEvalForm` bundles the claim, the multivariate poly,
...
structure SumcheckProtocolStatementEvalForm
...
def sumcheckClaimIsCorrectEvalForm {𝔽 : Type} {n d : ℕ}
    ...
    (st : SumcheckProtocolStatementEvalForm 𝔽 n d) : Prop :=
...
noncomputable def sumcheckProtocolEvalForm
    ...
    PublicCoinProtocol (SumcheckProtocolStatementEvalForm 𝔽 n d) 𝔽 n where
...
theorem sumOnDomain_honest_eq_symbolic
    {𝔽 : Type} {n d : ℕ} [Field 𝔽] [Fintype 𝔽] [DecidableEq 𝔽]
    (st : SumcheckProtocolStatementEvalForm 𝔽 n d)
    (i : Fin n)
    (challenges : Fin i.val → 𝔽) :
...
private noncomputable def honestEvalFormTranscript
    {𝔽 : Type} {n d : ℕ} [Field 𝔽] [Fintype 𝔽] [DecidableEq 𝔽]
    (st : SumcheckProtocolStatementEvalForm 𝔽 n d) (r : Fin n → 𝔽) :
...
private noncomputable def honestSymbolicTranscript
    ...
    (st_sym : SumcheckProtocolStatement 𝔽 n) (r : Fin n → 𝔽) :
    Transcript 𝔽 n :=
  { roundPolys := fun i =>
...
noncomputable def liftEvalProverToSymbolic
    ...
    SumcheckProtocolStatementEvalForm 𝔽 n d → Prover (sumcheckProtocol (𝔽 := 𝔽) (n := n)) :=
  fun st0 => {
    respond := fun _ i chs =>
...
/-- SumcheckProtocol claim correctness coincides on eval-form and symbolic
...
theorem sumcheck_hasSoundnessError_evalForm_uniform
  ...
  set st_sym : SumcheckProtocolStatement 𝔽 n := evalFormToStatement st

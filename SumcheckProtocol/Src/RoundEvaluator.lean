...
`SumcheckProtocolStatementEvalForm`, which itself carries a single `polynomial`.
This file provides the shared evaluator shape; downstream files (such as
`SumcheckProtocol/IP/InnerProductNative.lean`) provide the bridges that let a
...
`SumcheckProtocolStatementEvalForm.degree_le` field is itself stated per-`Fin n`,
...
`SumcheckProtocol/Src/EvalForm.lean::honestProverMessageEvalsAt`, which calls
...
* `innerProductEvaluator f g hf hg` — `d = 2` evaluator backed by the
  ...
  closed by `degreeOf_mul_le_c` from `SumcheckProtocol/IP/SharpSAT/Degree.lean`,

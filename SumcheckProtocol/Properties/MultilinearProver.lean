...
`SumcheckProtocol/Src/MultilinearProver.lean` against the symbolic spec.
...
/-- **Theorem (a) — narrowed (table-internal form).**
    ...
    and `… 1` from `SumcheckProtocol/Src/EvalForm.lean` is a spec-rewrite step that
    consumes the MSB↔LSB convention layer (`SumcheckProtocol/Src/Convention.lean`).

...
/-!
# CoefficientProver bridge: `RoundPolyEvaluator` → `SumcheckStatementEvalForm`
...
`SumcheckStatementEvalForm 𝔽 n d`. The degree bound transfers from the
...
/--
Plumb a `RoundPolyEvaluator` into a `SumcheckStatementEvalForm`. The
...
def toEvalFormStatement {n d : ℕ}
    ...
    SumcheckStatementEvalForm 𝔽 n d where
  domain := domain
  claim := claim

...
def commit (c : CappedMerkleCommitment H S) (msg : List (MerkleHasher.Symbol H))
    (_seed : ULift.{0} UInt64) : List (MerkleHasher.Digest H) × Trapdoor H S :=
  let mc := c.inner
...
namespace VectorCommitment.Tests.Capped

open CappedMerkleCommitment

-- Round-trip tests using a local demo hasher over `ZMod 65521`. Mirrors
-- the demo hasher used by `VectorCommitment/Tests/SchemeTests.lean`.
...
end VectorCommitment.Tests.Capped

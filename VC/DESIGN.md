# `VC` — Vector commitments + Merkle tree, in Lean
...
2. Be runnable (`lemma … := by native_decide` over a concrete hasher instance) so `VC` can serve as a correctness oracle for the Rust crate's outputs on small examples.
...
8. **Computability is non-negotiable for `Src/`.** Everything under `VC/Src/` is `def` (not `noncomputable`) so `lake build` of `VC.Tests` exercises real round-trips. Probability and information-theoretic statements live under `VC/Properties/` and may be `noncomputable`.
...
```
LeanStuff/
├── VC.lean                                    -- umbrella, parallel to Sumcheck.lean / InteractiveProtocol.lean
├── VC/
│   ├── DESIGN.md                              -- this file
│   ├── HIDING.md
│   ├── ROADMAP.md
...
`lakefile.lean` gains one entry: `lean_lib «VC» where`. No new external dependencies — Mathlib `v4.28.0` already supplies `PMF`, `Finset`, `ZMod`, `ByteArray`.
...
*How this interacts with the two-class hiding split.* The `Salt` associated type determines *which* of the `VC.Trait` typeclasses `MerkleCommitment H S` implements. `H.Salt = Unit` ⇒ non-hiding only; `H.Salt ≠ Unit` ⇒ both. Full analysis in [HIDING.md](HIDING.md).
...
class VectorCommitment (V : Type*) where
  ...
  setup  : (maxLen maxQueries : ℕ) → ULift UInt64 → UniversalParams
...
`ProverChannel` / `VerifierChannel` are deliberately omitted from L0. The Rust trait threads transcripts through the channel for Fiat-Shamir composition. We defer that — when integrating with [`InteractiveProtocol/Src/Protocol.lean`](../InteractiveProtocol/Src/Protocol.lean), VC's `commit` output becomes a `ProverMessage`. The channel-threaded variant arrives in L2 (see [ROADMAP.md](ROADMAP.md)).

### 3.4 Computability — `Src/` is `def`, `Properties/` is `noncomputable`

Tests use `lemma … := by native_decide` (matching [`Sumcheck/Tests/TranscriptTests.lean`](../Sumcheck/Tests/TranscriptTests.lean)) so `commit` / `open` / `check` round-trips are verified at build time. `OpeningProof` carries `DecidableEq` so equality assertions work under `native_decide`. The demo hasher in `VC/Tests/HasherTests.lean` is a plain `def` over `ZMod 65521` — a prime small enough to fit in `UInt32`, large enough to make collisions unlikely on tiny inputs, and trivial to mirror in Rust.
...
|---|---|---|
| 1 | `Commit : Σ^ℓ → (C, td)` | `VC/Src/Merkle/Scheme.lean :: MerkleCommitment.commit` |
| 2 | `Open : (td, I) → π` | `VC/Src/Merkle/Scheme.lean :: MerkleCommitment.open` |
| 3 | `Check : (C, I, m[I], π) → {0,1}` | `VC/Src/Merkle/Scheme.lean :: MerkleCommitment.check` |
...
- [`InteractiveProtocol/Src/Protocol.lean :: PublicCoinProtocol`](../InteractiveProtocol/Src/Protocol.lean) — anchors the future channel-threaded VC variant in L2; not used at L0.
...
- **Verified compilation to Rust.** `VC` is an oracle, not a code generator.

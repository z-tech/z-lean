...
**This branch delivers L0 through L10 essentially in full.** Only ONE `sorry` remains across the entire `VC` module — the deferred `mt_equivocation` (book §12.7) which requires probabilistic ZK simulator machinery beyond the placeholder RO.
...
|---|---|---|
| **L0** | Skeleton: typeclasses, data types, and `sorry`'d theorem statements for every book result in §12 + §20 + §12.7. Four design docs (DESIGN, HIDING, ROADMAP, USAGE). All file homes from [DESIGN.md §2](DESIGN.md#2-module-layout) exist. The demo `ZMod 65521` hasher is wired into `HasherTests.lean` only — `commit`/`open`/`check` bodies remain `sorry`. | `lake build` of `«VC»` passes. Every lemma/theorem typechecks modulo `sorry`. One smoke `lemma` on the empty/size-0 round-trip compiles via `native_decide`. |
...
1. The four design docs exist: `VC/DESIGN.md`, `VC/HIDING.md`, `VC/ROADMAP.md`, `VC/USAGE.md`.
2. Every file path listed in [DESIGN.md §2](DESIGN.md#2-module-layout) exists (modulo placeholder bodies).
3. `lakefile.lean` contains `lean_lib «VC» where`.
...
- **Parallel `commit`.** Lean has no rayon. Single-threaded performance is sufficient — `VC` is an oracle for small examples, not a production prover.
- **Byte-level wire-format compatibility with `ark-serialize`.** `VC` cross-checks Rust output by *value* (digest equality on small tests), not by byte stream. If wire compatibility ever matters, a `WireEncode` typeclass lands as a separate module — not a milestone.
...
- **Verified compilation to Rust.** `VC` is a spec, not a code generator.

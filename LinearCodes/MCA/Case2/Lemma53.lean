/-
# Lemma 5.3 (MDS aggregate-counting) for Case 2 (BCGM25 Theorem 6.1)

The big BCGM25 Lemma 5.3 proof and its column-difference helpers. Imports
the MDS-bridge sub-targets from `Case2/MDSBridge.lean` (specifically the
`degree_bound_at_non_Ttilde` that the L5.3 counting argument consumes).

* A2-A6: column-difference helpers (`colDiff`, `mem_Ttilde_iff_colDiff_zero`,
  `combine_eq_iff_dotMap_colDiff_zero`, `bad_pair_count_per_coord_le`).
* L5.3: `Ttilde_card_gt_of_MDS_aggregate` — paper-tight at the
  integer-honest BCH+25 bound (see file header in
